...
\revise{People refer to this mapping between form and meaning as the \tit{sentential force} or \tit{Mood} (\cite{chierchia1990textbook, portner2018}). This category is systematically related to clause types, i.e. the forms of sentences, on the one hand, and speech acts, i.e. the pragmatic functions that the sentences are used to perform, on the other. In this dissertation, I will mostly consider the learning of \tit{matrix} clauses and their associated sentential force.  }%Observations of only the formal distribution or only the pragmatic function had the potential to mislead, and so the learners need both to mutually constrain each other.
...
\section{Learning \revise{matrix} clause types}
\label{sec:intro:cl}
