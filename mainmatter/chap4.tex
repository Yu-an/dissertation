...
\notdone{But the mere existence of these morphosyntactic differences does not make syntactic bootstrapping a foregone conclusion. A major problem lies in the fact that the overt expression of these elements is optional (we set aside the question of whether they are syntactically present but silent). For one, Mandarin is a pro-drop language, so subjects (as well as objects) can be omitted from declarative main clauses and complement clauses under the right context. Likewise, not all utterances require modals or aspect markers. Similarly, imperatives can appear with overt subjects (as can imperatives in other languages, like English). This optionality means that complement clauses of belief and desire verbs can be string-identical (9), as can declarative and imperative clauses (10). To the extent that belief and desire verbs have complements that are superficially indistinguishable from each other and are also indistinguishable from declarative and other clause types, this presents a serious problem for the syntactic bootstrapping hypothesis.}
...
\notdone{ it’s interesting to look at Mandarin, namely that clause types look very different than in English in ways that could make their acquisition even harder. You should also discuss the controversy about imperatives in Mandarin. Isn’t there some literature on this? Ultimately, the model fails to find imperatives, even with pragmatic info, so it seems something important to stress and to somehow anticipate in your introduction of the chapter. [and probably even in chapter 1, where you should explain that you’ll look at two languages, one, English, with fairly clear syntactic hallmarks distinguishing the there main clause types, and one, Mandarin, where the different clause types may look more similar to one another). Your discussion seems to assume that the pragmatically informed learner succeeded. But did it? }