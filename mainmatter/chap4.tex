...
In theoretical discussions, it is extremely difficult to pin down what are imperatives in Mandarin. One diagnostic is the negative modal \tit{bie} (and its etymologically related modal \tit{beng}), which cannot occur with other values of $C^{0}$ (\cite{chao1968, lithompson}):
...
Another surface feature for English [imp] is [$-$ verb suffix], but Mandarin verbs do not have suffixes. Mandarin does have aspect morphemes \tit{le, zhe, guo}, but they can appear with [imp]:  
...
\revise{While adding pragmatic information improves the performance of the model, it still fails to identify all three clusters. Why are both models fail when simulating with Mandarin data? The problem seems to be that declaratives and imperatives simply do not look that different in the input. Even with its own marker \tit{bie}, there are few cases of sentences with this marker, which is not significant enough to differentiate the two clause types.  }% One possibility is that this is what the learners are doing: they mix declaratives with imperatives at the beginning   }
\revise{But it is possible that this is exactly what the learners are doing: they mix declaratives with imperatives at the beginning. As they hear more sentences with \tit{bie}, they can then have a dedicated category for imperatives. }

\revise{One way the performance could be improved is to add in the prosodic information. But this information could be a double-edge sword: with the complication of lexical tones, children might first need to determine what kind of acoustic features that are at the clausal level. }
